<?php
/**
 * Sixreps - Detect user agent class
 *
 * @author Sixreps
 */
 ...
 * Sixreps
 *
 * This class for detect what user agent that currently used by user and return the information
 *
 * @package Sixreps
 */
class SixrepsAgentDetect {

    /**
     * @var string Current user browser
     */
    protected $_user_browser;

    /**
     * @var string Current user browser accept
     */
    protected $_user_accept;

    /**
     * @var array Browser WAP identity list
     */
    protected $_wap_identity = array('wapbrowser','up.browser','up\/4','mib','cellphone','go.web', 'nokia',
                                     'panasonic','wap','wml-browser','wml');

    /**
     * @var array Browser PC identity list
     */
    protected $_pc_identity = array('mozilla','gecko','opera','omniweb','msie','konqueror','safari',
                                    'netpositive' ,'lynx' ,'elinks' ,'links' ,'w3m' ,'webtv' , 'amaya',
                                    'dillo' ,'ibrowse' ,'icab' ,'crazy browser' ,'internet explorer');

    /**
     * @var array Browser PSP identity list
     */
    protected $_psp_identity = array('PlayStation Portable');

    /**
     * @var array Browser Touch identity list
     */
    protected $_touch_identity = array('(iPhone.*Mobile|iPod|iTunes)', 'android',
     ...
     * @var array Browser Touch identity list
     */
    protected $_tablet_identity = array(
                                        # Tablet device
     ...
     * @return  void
     */
    public function __construct($user_browser = null, $user_accept = null) {
        if (!empty($user_browser)) {
            $this->_user_browser = strtolower($user_browser);
        } else {
            $this->_user_browser = strtolower($_SERVER['HTTP_USER_AGENT']);
        }

        if (!empty($user_accept)) {
            $this->_user_accept = strtolower($user_accept);
        } else {
            $this->_user_accept = strtolower($_SERVER['HTTP_ACCEPT']);
        }
    }
     ...
     * @param   array   $accept    HTTP Accept
     * @return  string  Browser type
     */
    public function check($browser = NULL, $accept = NULL)
    {
        if (!empty($browser)) {
            $this->_user_browser = strtolower($browser);
        }

        if (!empty($accept)) {
            $this->_user_accept = strtolower($accept);
        }
    ...
    protected function identify()
    {
        ...
        # Just for blackberry
        if(preg_match("/BlackBerry/i", $this->_user_browser)) {

            # Blackberry OS 6.0
            if (preg_match("/6\.0\../i", $this->_user_browser)) {
                $type = "blackberry";
            }

            # Blackberry OS 5.0
            if (preg_match("/5\.0\../i", $this->_user_browser)) {
                $type = "blackberry";
            }

            # Blackberry OS 4.7
            if (preg_match("/4\.7\../i", $this->_user_browser)) {
                $type = "blackberry";
            }

            # Blackberry OS 4.6
            if (preg_match("/4\.6\../i", $this->_user_browser)) {
                $type = "blackberry";
            }

            # Blackberry OS 4.5
            if (preg_match("/4\.5\../i", $this->_user_browser)) {
                $type = "blackberry";
            }

            # Blackberry OS 4.3
            if (preg_match("/4\.3\../i", $this->_user_browser)) {
                $type = "blackberry";
            }

            # Blackberry OS 4.2.1
            if (preg_match("/4\.2\.1/i", $this->_user_browser)) {
                $type = "blackberry";
            }

        } else if (stristr($this->_user_accept,'wml')) {
           // $type = "WML";
           $type = "wap";
        } else {
            if ($this->_browser_identity($this->_user_browser, $this->_wap_identity)){
                // $type = "WML";
                $type = "wap";
            } elseif ($this->_browser_identity($this->_user_browser, $this->_touch_identity)){
                // $type = "iphone";
                $type = "phone";
            } elseif ($this->_browser_identity($this->_user_browser, $this->_tablet_identity)){
                // $type = "iphone";
                $type = "tablet";
            } elseif ($this->_browser_identity($this->_user_browser, $this->_psp_identity)){
                // $type = "PSP";
                $type = "phone";
            } elseif ($this->_browser_identity($this->_user_browser, $this->_pc_identity)){
                // $type = "PC";
                $type = "www";
            } else {
                // $type = "WML";
    ...
    /**
     * Check browser identity is match to our browser type DB
     *
     ...
     */
    protected function _browser_identity($from, $identities)
    {
        foreach ($identities as $identity) {
            if(preg_match('/'.$identity.'/is', $from)){
                return true;
            }
        }
